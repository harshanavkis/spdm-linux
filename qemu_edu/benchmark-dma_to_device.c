...
MODULE_PARM_DESC(count_ops, "Number of DMA operations");
...
start = ktime_get();
	for (long i = 0; i < count_ops; ++i) {
		dma_sync_single_for_device(&(dev->dev), dma_handle, dma_size, DMA_BIDIRECTIONAL);
...
pr_info("time measured: %lu;%llu end\n", dma_size, ((u64) ktime_to_ns(end) - (u64) ktime_to_ns(start)) / count_ops);
