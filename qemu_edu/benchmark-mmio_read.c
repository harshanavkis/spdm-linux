...
static long mmio_size = 1;
module_param(mmio_size, long, 0);
MODULE_PARM_DESC(mmio_size, "The byte size of the MMIO access. Must be one of {1, 2, 4, 8}");
		...
		switch (mmio_size) {
			case 1:
				for (int i = 0; i < count_ops; ++i) {
					//start = ktime_get();
					readb(mmio + IO_DMA_CNT);
					//end = ktime_get();
					//agg += ktime_to_ns(end) - ktime_to_ns(start);
				}
				//pr_info("time measured: %lu;%llu end\n", mmio_size, agg / count_ops);
				//agg = 0;
				break;
			case 2:
				for (int i = 0; i < count_ops; ++i) {
					//start = ktime_get();
					readw(mmio + IO_DMA_CNT);
					//end = ktime_get();
					//agg += ktime_to_ns(end) - ktime_to_ns(start);
				}
				//pr_info("time measured: %lu;%llu end\n", mmio_size, agg / count_ops);
