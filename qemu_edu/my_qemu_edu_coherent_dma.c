...
*/
	...
	/* Optional sanity checks. The PCI is ready now, all of this could also be called from fops. */
	{
		...
		}
		{
		    ...
		    /*** First buffer ***/
		    actual1 = dma_alloc_coherent(&(dev->dev), SIZE1, &dma_handle1, 0);
		    if (!dma_handle1) {
		    ...
		    /*** Second buffer ***/
		    actual2 = dma_alloc_coherent(&(dev->dev), SIZE2, &dma_handle2, 0);
		    if (!dma_handle2) {
		...
		unmap_3:
		    dma_free_coherent(&(dev->dev), SIZE2, actual2, dma_handle2);
		unmap_3_1:
		    dma_free_coherent(&(dev->dev), SIZE1, actual1, dma_handle1);
		kfree_3:
		    kfree(expected);
		}
