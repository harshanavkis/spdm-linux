...
static int __init vsock_server_init(void) {
    ...
    pr_info("Vsock server module loaded\n");
    return 0;
}

static void __exit vsock_server_exit(void) {
    if (vsock_server_thread) {
        kthread_stop(vsock_server_thread);
    }

    pr_info("Vsock server module unloaded\n");
}

module_init(vsock_server_init);
module_exit(vsock_server_exit);
